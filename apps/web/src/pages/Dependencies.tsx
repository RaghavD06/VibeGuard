...
import { Package, ArrowUpCircle, RefreshCw, ShieldCheck } from 'lucide-react';
import { toast } from 'sonner';

export function Dependencies() {
  const { selectedRepo } = useRepo();
  const collection = useCollection('/api/findings?kind=dependencies', selectedRepo);
  const dependencies = collection.data;
  const fetchDependencies = collection.refresh;
  ...
  const handleScanDependencies = () => {
    fetchDependencies();
    toast.success('Dashboard results refreshed', {
      description: 'Run the VibeGuard CLI to generate a new dependency audit.'
    });
  };

  const handleUpdatePackage = (title: string) => {
    toast.info(`Review the update for ${title}`, {
      description: 'VibeGuard does not modify repository dependencies from the dashboard.'
    });
  };
              ...
              return (
                <div key={idx} className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 flex flex-col hover:border-[#00E599]/30 transition-all">
                  ...
                  <div className="pt-4 border-t border-white/[0.08] flex justify-between items-center mt-auto">
                    ...
                    <button
                      onClick={() => handleUpdatePackage(dep.title)}
                      className="text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer text-[#00E599] hover:underline"
                    >
