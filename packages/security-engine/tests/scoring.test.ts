...
describe('Deterministic Scoring Engine (0-100)', () => {
  ...
  it('should return score 100 and grade A for a clean assessed scan', () => {
    const result = calculateScore([], { code: true });
  ...
  it('should not score a scan with no assessed domains', () => {
    const result = calculateScore([]);
    expect(result.score).toBeNull();
    expect(result.grade).toBe('UNASSESSED');
  ...
  it('should deduct 3 points per medium finding (e.g. 2 mediums = 94, Grade A)', () => {
    ...
    const result = calculateScore(findings, { code: true });
